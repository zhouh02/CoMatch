#!/usr/bin/env python3
"""
Precompute OneFormer semantic segmentation for ScanNet test images.

This script runs OneFormer on all unique images from the ScanNet test set
...
Usage:
    ...
    python scripts/precompute_oneformer_scannet_semantics.py \
        --data_cfg configs/data/scannet_test_1500.py \
        --cache_dir outputs/scannet_semantic_cache_joint \
        --model_id shi-labs/oneformer_ade20k_swin_large \
        --device cuda \
        --joint_mode
...
def collect_scannet_pairs(data_cfg_path: str) -> Tuple[List[Tuple[str, str, str]], int]:
    """
    Collect image pairs from ScanNet test configuration.
    ...
    for info in tqdm(scene_info_list, desc="Loading scene datasets"):
        ...
        datasets.append(
            ScanNetDataset(
                ...
                img_resize=(config.DATASET.SCAN_IMG_RESIZEX, config.DATASET.SCAN_IMG_RESIZEY),
            )
        )

    num_pairs = sum(len(ds) for ds in datasets)

    pair_entries = []
    for ds in datasets:
        for idx in range(len(ds)):
            data = ds[idx]
            rel_path_0 = data['pair_names'][0]
            rel_path_1 = data['pair_names'][1]
...
def collect_scannet_test_images(data_cfg_path: str) -> Tuple[List[Tuple[str, str]], int, int]:
    """
    Collect unique image paths from ScanNet test configuration.

    Uses the actual ScanNetDataset to get pair_names, guaranteeing
    the same path format that will be used during CoMatch evaluation.

    Args:
        data_cfg_path: Path to ScanNet data config file
...
def main():
    parser = argparse.ArgumentParser(
        description="Precompute OneFormer semantic segmentation for ScanNet test images"
    )
    parser.add_argument(
        '--data_cfg',
        type=str,
        help='ScanNet data config path (e.g., configs/data/scannet_test_1500.py)'
    )
    parser.add_argument(
        '--cache_dir',
        type=str,
        default='outputs/scannet_semantic_cache',
    ...
    if args.validate_only:
        print("=" * 60)
        print("Validating ScanNet Semantic Cache")
        print("=" * 60)
    ...
    if args.joint_mode:
        ...
        print("=" * 60)
        pair_entries, num_pairs = collect_scannet_pairs(args.data_cfg)
    ...
    if args.image_list:
        ...
        print(f"Loading ScanNet test images from config: {args.data_cfg}")
        image_entries, num_pairs, num_unique = collect_scannet_test_images(args.data_cfg)
